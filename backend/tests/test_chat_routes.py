...
def test_upload_creates_document_sections(client, storage):
    ...
    content = (
        ...
        'Applicants must complete MATH 113 and maintain a GPA of 2.5.\n\n'
        'Semester Schedule\n'
        'Semester 1 includes CSCI 104 and ENGL 101.'
    )
...
def test_retrieval_failure_returns_grounded_no_evidence(client, uploaded_source):
    response = client.post(
        ...
        json={
            'conversation_id': uploaded_source['conversation_id'],
            'source_id': uploaded_source['source_id'],
            'message': 'compare astrophysics and marine law requirements',
        },
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_schedule_query_hits_schedule_section(client):
    ...
    file_content = (
        ...
        'Semester Schedule\n'
        'Semester 1: CSCI 104, MATH 113\n'
        'Semester 2: CSCI 112, PHYS 110\n'
    )
...
def test_catalog_question_can_override_selected_upload_source(client, uploaded_source, catalog_source):
    response = client.post(
        ...
        json={
            'conversation_id': uploaded_source['conversation_id'],
            'source_id': uploaded_source['source_id'],
            'message': 'what majors does aurak catalog offer',
        },
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_mixed_question_uses_upload_and_catalog_context(client, catalog_source):
    ...
    response = client.post(
        ...
        json={
            'conversation_id': conversation_id,
            'source_id': source_id,
            'message': 'using my syllabus and aurak catalog, compare requirements with my current week',
        },
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_google_sheet_exam_schedule_questions_retrieve_rows(client, monkeypatch):
    ...
    queries = [
        'when is the exam for CSCI 104',
        'where is the final for MATH 113',
...
def test_google_sheet_follow_up_keeps_source_context(client, monkeypatch):
    ...
    first = client.post(
        ...
        json={
            'conversation_id': conversation_id,
            'source_id': source_id,
            'message': 'when is the exam for CSCI 104',
        },
    )
    assert first.status_code == 200
...
def test_partial_exact_location_question_hedges_cleanly(client):
    ...
    file_content = (
        'Course Exam Schedule\n'
        'CSCI 104 Midterm Exam: 2026-05-14 at 10:00 AM\n'
    ...
    response = client.post(
        ...
        json={
            'conversation_id': conversation_id,
            'source_id': source_id,
            'message': 'where is the exam for CSCI 104',
        },
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_mixed_source_exact_conflict_is_called_out(client, storage):
    ...
    storage.upsert_source('catalog', 'AURAK Catalog (All Catalog PDFs)', 'catalog', None)
    retrieval_service.index_source_document(
        source_id='catalog',
        transcript='Official AI exam notice. CSCI 104 Midterm Exam: 2026-05-16 at 10:00 AM.',
        sections=[
            {
                'section_title': 'Official Exam Schedule',
                'section_type': 'schedule',
                'section_text': 'Official AI exam notice. CSCI 104 Midterm Exam: 2026-05-16 at 10:00 AM.',
                'keywords': ['exam', 'schedule', 'CSCI 104'],
                'page_start': 18,
                'page_end': 18,
                'facts': {
                    'dates': ['2026-05-16'],
    ...
    conversation_id = created.json()['conversation']['id']
    upload_text = 'My syllabus says CSCI 104 Midterm Exam: 2026-05-14 at 10:00 AM.'
    upload = client.post(
    ...
    response = client.post(
        ...
        json={
            'conversation_id': conversation_id,
            'source_id': source_id,
            'message': 'based on my syllabus and the aurak catalog, when is the exam for CSCI 104',
        },
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_live_external_requests_are_limited_honestly(client):
    response = client.post(
        '/api/chat',
        json={
            'message': 'what is the weather today in ras al khaimah',
        },
    )
    assert response.status_code == 200
    payload = response.json()
